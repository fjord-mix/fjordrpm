function [QVi0, QTi0, QSi0] = get_iceberg_fluxes(i, p, s)

% GET_ICEBERG_FLUXES Compute mixing fluxes for the zmodel.
%   [QVI0, QTI0, QSI0] = GET_ICEBERG_FLUXES(I, P, S)
%   computes the iceberg fluxes QVI0, QTI0, QSI0
...
if p.M0==0 
    ...
    Qent = p.U0*p.alphaI^(2/3)*meltflux.^(1/3).*gmelt.^(1/3).*H0.*I0.^(2/3);
    ...
    meltcont = [scalefac(2:end);0].*[meltflux(2:end);0] + (1-scalefac).*meltflux;
